...
class PostPagesTests(TestCase):
    ...
    def test_urls_status_code(self):
        urls_names = [
            ...
            ['/about/author/', self.guest_client, 200],
            ['/about/tech/', self.guest_client, 200],
            [f'/group/{self.group.slug}/', self.guest_client, 200],
            [f'/profile/{self.user_author}/', self.guest_client, 200],
            [f'/posts/{self.post.id}/', self.guest_client, 200],

            [f'/posts/{self.post.id}/edit/', self.post_author, 200],
            [f'/posts/{self.post.id}/edit/', self.guest_client, 302],

            ['/create/', self.authorized_client, 200],
            ['/create/', self.guest_client, 302],

            # Запрос к страница unixisting_page вернет ошибку 404
            ['/unixisting_page/', self.guest_client, 404],
        ]
